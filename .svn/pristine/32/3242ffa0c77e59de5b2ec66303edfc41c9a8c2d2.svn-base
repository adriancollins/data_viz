(function() {
    var a = 800, //left align	|DEFAULT:8000
        c = 800, //	|DEFAULT:8000
        h = c, //	|DEFAULT:c
        U = 200, //this is the centre list width	|DEFAULT:200
		K = 20, //Y offset factor for centre list height	|DEFAULT:16
        S = 20, //	|DEFAULT:20
        s = 8, //width of connector path	|DEFAULT:8
        R = 100, //radius of entire graphic	|DEFAULT:110
        J = 25, //object seperation	|DEFAULT:30
        o = 15, //	|DEFAULT:15
        t = 10, //	|DEFAULT:10
        w = 900, //bounce time (ms)	|DEFAULT:1000
        F = "elastic",
		N = "#0080B6",  //Hoverover color	|Defaukt:"#115a9e"
		big_cric_col = "#000" //big circle color
		small_circ_col_grp = "#0080B6", //small circle color	|Default: #000
		small_teams_col = "#FFF",
		small_teams_outline_col = "#0080B6",
		big_circ_size = 100, //radius of big circles
		small_circ_size = 8, //scaled circle size for groups with multiples
		small_circ_size2 = 6, // small circles on front page
		font_size_items = 17, //font size of menu items
		rect_color = "#333740", //DEFAULT:#000000
		team_conn_color = "#0080B6",
		conn_line_color = "#777676";
...
//    }).on("click", O); //this function controls the "back" funcitonality from clicking on white space
   ...
   while(end < start + ms) {
    ...
    function M() {
        ...
        D(T.get("episodes"));
		//D(T.get("episodes"));b(d3.merge([T.get("themes"),T.get("perspectives")]));C([]);m(P);Q.html('Made with in Cork');n();z();
    ...
    function b(X) {
        ...
        X.selectAll("circle").transition().duration(w).ease(F).attr("r", function(Z) {
            if (Z == L.node) {
                return big_circ_size
            } else {
                if (Z.isGroup) {
					return small_circ_size + Z.count
                    //return 7 + Z.count
                } else {
					return small_circ_size2
                    //return 4.5
                }
            }
        });
    ...
    function C(Z) {
      ...
      //var aa = Y.append("text").text( function(ae) {ae.slug});
            //var aa = Y.append("a").attr("xlink:href", function(ae) {
            //    return "/" + ae.slug
            //});

            var aa = Y.append("image").attr("xlink:href", function(ae){return ae.pic}).attr("y", -100).attr("x", -100).attr("width", 200).attr("height", 200);

			//aa.append("image").attr("xlink:href","https://www.bestpersonnel.ie/wp-content/uploads/2017/11/Sani-Sebastian.png").attr("y", -100).attr("x", -100).attr("width", 200).attr("height", 200);
			//aa.append("image").attr("xlink:href",ae.pic).attr("y", -100).attr("x", -100).attr("width", 200).attr("height", 200);

		//	aa.append("text").attr("fill", N).attr("text-anchor", "middle").attr("y", (o + t) * -1).text(function(ae) {
			     //return ae.pic
			//    return "INVENTOR " + ae.episode
      //  });
      aa.append("text").attr("text-anchor", "middle").attr("y", 0).attr("fill", "#000").style("font-size", "15px").text(function(ae) {return ae.name});
        } else {
            if (ab && ab.type === "theme") {

                Y.append("text").attr("fill", "#aaa").attr("text-anchor", "middle").attr("y", (o + t) * -1).text("PATENT")
            } else {
                if (ab && ab.type === "perspective") {
                    ...
                    Y.append("text").attr("fill", "#aaa").attr("text-anchor", "middle").attr("y", (o + t) * -1).text	("TEAM");
                    ad.exit().remove()
                }
            }
        }
        ...
        X.enter().append("text").attr("text-anchor", "start").attr("x", a / -2 + t).attr("y", c / 2 - t).text("home").attr("class", "all-episodes").on("click", O);
    ...
    function m(Y) {
        ...
        }).attr("stroke", team_conn_color).attr("stroke-width", 1.5).transition().duration(w).ease(F).attr("d", function(Z) {
            return v([
                [Z.x1, Z.y1],
                [Z.xx, Z.yy],
                [Z.x2, Z.y2]
    ...
    function z() {
        f.selectAll("rect").attr("fill", function(X) {
            return l(X, rect_color, N, "#000")
        });
        B.selectAll("path").attr("stroke", function(X) {
            return l(X, conn_line_color, N, "#aaa")
        ...
        E.selectAll("circle").attr("fill", function(X) {
            if (X === L.node) {
                //return "#000"
				return big_cric_col
				//return small_circ_col_grp
            } else {
                if (X.type === "theme") {
                    //return l(X, "#666", N, "#000")
					return(X, "#000",N, small_circ_col_grp)
                } else {
                    if (X.type === "perspective") {
                        //return "#fff"
					   //IF group
						//return small_circ_col_grp
						return small_teams_col
                    }
                }
            }
            //return l(X, "#000", N, "#999")
			return l(X, small_circ_col_grp, N, "#999") //individual ppl small ball colors
            ...
            if (X === L.node) {
                ...
                if (X.type === "theme") {
					//return small_circ_col_grp
                    return "#000" //main page small circle outline
					
					
                } else {
                    if (X.type === "perspective") {
                        return l(X, small_teams_outline_col, N, "#000") //outline of groups
